...
try:
    ...
    print >> sys.stderr, " You don't seem to have gdata installed!\n Fetch "\
        "it from http://code.google.com/p/gdata-python-client/downloads/list "\
        "and install it anywhere where python can find it.\n Best let the "\
        "setup-tools do their job..."
    sys.exit(-1)
...
if len(config.read([
    '/etc/googlebackup/googlebackup.conf',
    os.path.expanduser('~/.config/googlebackup/googlebackup.conf')
])) < 1:
    saveconfig()

description = """
%prog fetches all documents from google-docs to a local directory.
Its primary use is for local backups. That is in collaboration with whatever
backup-mechanism you already use...
%prog will fetch the last version of a document from google and overwrite
whatever older version you had on disk. To actually have a longer backup, you
have to add this directory into the tree your backup-application backs up. Best
is to make your backup-app to run %prog before the real backup-run.
"""
...
parser.add_option(
    '-q', '--quiet',
    help='Run really quiet. No output apart from important errors (which are '
    'sent to stderr).',
    action='store_true',
    default=False
)
parser.add_option(
    '-i', '--interactive',
    help='Run interactively if needed.',
    dest='interactive',
    default=True,
    action='store_true'
)
parser.add_option(
    '-n', '--non-interactive',
    help='Don\'t run interactive. The apps fails if there is no valid login '
    'token for google and no username/passwd in the config-file.',
    dest='interactive',
    action='store_false'
)
...
if not os.path.exists(backupdir):
    raise OSError(
        'The directory \'%s\' for the backup files doesn\'t exist.\n Either '
        'create that dir or change the value of backupdir in the config-file.'
        % backupdir
    )
...
try:
    ...
    spreadsheetservice.SetClientLoginToken(
        config.get('googlebackup', 'spreadsheettoken')
    )
...
if not loginworked:
    ...
    passwd = ''
    if (
        config.has_option('googlebackup', 'username') and
        config.has_option('googlebackup', 'passwd')
    ):
    ...
    try:
        docsservice.ClientLogin(
            username, passwd,
            source="Python Google Docs Backup"
        )
        spreadsheetservice.ClientLogin(
            username, passwd,
            source="Python Google Docs Backup"
        )
        config.set(
            'googlebackup', 'docstoken',
            docsservice.GetClientLoginToken()
        )
        config.set(
            'googlebackup', 'spreadsheettoken',
            spreadsheetservice.GetClientLoginToken()
        )
        loginworked = True
        saveconfig()
    except:
...
if not loginworked:
    ...
    if not options.interactive:
        print >> sys.stderr, "You have to run this app interactively at " \
            "least once to enter username and password and retrieve a valid " \
            "login token from google.\nOr add username and passwd to the " \
            "configuration file. (Note that these aren't saved in " \
            "interactive sessions.)"
    sys.exit(-1)

lastrun = datetime.datetime.strptime(
    config.get('googlebackup', 'lastrun').split('.')[0],
    '%Y-%m-%dT%H:%M:%S'
)
...
for entry in alldocuments:
    ...
    for link in entry.link:
        ...
        if verbose:
            print '  Has link: %s, %s, %s, %s' % (
                link.rel, link.title, link.type, link.href
            )
    ...
    published = datetime.datetime.strptime(
        entry.published.text.split('.')[0],
        '%Y-%m-%dT%H:%M:%S'
    )
    updated = datetime.datetime.strptime(
        entry.updated.text.split('.')[0],
        '%Y-%m-%dT%H:%M:%S'
    )
    if lastrun < updated or not os.path.exists(filename):
        ...
        if ending is endings['spreadsheet']:
            #
            # downloading spreadsheets requires the authentication token from
            # the spreadsheet service.
            #
            docstoken = docsservice.GetClientLoginToken()
            docsservice.SetClientLoginToken(
                spreadsheetservice.GetClientLoginToken()
            )
            try:
            ...
            #
            # Anything else will just work with the normal docservice
            # auth-token
            #
            try:
                downloadfile(docsservice, entry, filename, ending)
